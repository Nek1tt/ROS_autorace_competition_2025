...
class KonusDetection(Node):
    def __init__(self):
        ...
        # === ТОЛЬКО ОРАНЖЕВЫЙ/КРАСНЫЙ (БЕЗ ЖЕЛТОГО И БЕЛОГО!) ===
        # Диапазон 1: оранжевый (5-20 в HSV, исключая желтый >25)
        self.lower_orange1 = np.array([0, 80, 80])
        self.upper_orange1 = np.array([20, 255, 255])
        
        # Диапазон 2: красный на противоположном конце спектра
    ...
    def process_cones(self, image, debug_img):
        ...
        # ROI: нижние 60% кадра (конусы на дороге, не в небе)
        roi_start = int(h * 0.4)
        roi = image[roi_start:, :]
        ...
        # === МАСКА ТОЛЬКО ДЛЯ ОРАНЖЕВОГО (исключаем желтый и белый) ===
        ...
        if contours:
            for cnt in contours:
                ...
                # Фильтр по площади (от 300 до 50000 пикселей)
                ...
                # 1. Пропорции: конус вертикальный (w/h от 0.3 до 1.0)
                if not (0.3 <= aspect_ratio <= 1.0):
                    continue
                
                # 2. Минимальная высота (конус должен быть достаточно высоким)
                if h_box < 20:
                    continue
                
                # 3. Проверка выпуклости (конус имеет простую форму)
                ...
                # 4. Проверка позиции: не берем объекты слишком близко к краям
                margin = int(w * 0.05)
